...
class Data:
    ...
    def read(self, filepath):
        ...
        with open(filepath) as csvfile:
            ...
            numeric = reader.__next__()

            for i, ele in enumerate(headers):
                headers[i] = ele.strip()

            for i, ele in enumerate(numeric):
                numeric[i] = ele.strip()
                if numeric[i] != 'numeric':
                    removing.append(i)
            if 'numeric' not in numeric:
                raise Exception(
                    "No numeric data in the dataset or the datatype column is not present in the dataset!")

            for index in sorted(removing, reverse=True):
                del headers[index]
            self.headers = headers

            self.header2col = {k: v for v, k in enumerate(headers)}

            for row in reader:
                for index in sorted(removing, reverse=True):
                    del row[index]
                for ele in range(len(row)):
                    row[ele] = float(row[ele])
    ...
    def get_header_indices(self, headers):
        ...
        '''
        if len(headers) == 0:
            raise Exception("No headers in the headers provided")
        else:
    ...
    def select_data(self, headers, rows=[]):
        ...
        #2 of self.data. If rows is not [] (say =[0, 2, 5]), then we do the same thing,
        column
